class Sprite {
  ...
  constructor(x, y, width, height, elements){
    ...
    this.width = width;
    this.height = height;
    this.ratio = this.elements.ratio;
    this.adjustValue();
  }
  ...
  display(){
    ...
    this.elements.ctx.translate(this.adjustX + this.adjustWidth / 2, this.adjustY + this.adjustHeight / 2);
    this.elements.ctx.rotate((Math.PI / 180) * this.rotation);
    this.elements.ctx.translate(-(this.adjustX + this.adjustWidth / 2), -(this.adjustY + this.adjustHeight / 2));
    if(this.status === 'in_use_as_image') {
      this.elements.ctx.drawImage(this.img, this.adjustX, this.adjustY, this.adjustWidth, this.adjustHeight);
    } else if(this.status === 'in_use_as_spritesheet'){
      this.elements.ctx.drawImage(this.img, this.sheet_option.sx, this.sheet_option.sy, this.sheet_option.width, this.sheet_option.height, this.adjustX, this.adjustY, this.adjustWidth, this.adjustHeight);
    }
    else if(this.status === 'in_use_as_default') {
      this.elements.ctx.fillStyle = this.default_option.rect_style;
      if('form' in this.default_option === true){
        if(this.default_option.form === 'circle'){
          this.elements.ctx.beginPath();
          this.elements.ctx.arc(this.adjustX + (this.adjustWidth / 2), this.adjustY + (this.adjustHeight / 2), this.adjustWidth / 2, 0, 2 * Math.PI, true);
          this.elements.ctx.fill();
        } else if(this.default_option.form === 'rect') this.elements.ctx.fillRect(this.adjustX, this.adjustY, this.adjustWidth, this.adjustHeight);
      } else this.elements.ctx.fillRect(this.adjustX, this.adjustY, this.adjustWidth, this.adjustHeight);
      if('text_content' in this.default_option === true){
        if('text_color' in this.default_option === true) this.elements.ctx.fillStyle = this.default_option.text_color;
        else this.elements.ctx.fillStyle = 'rgba(0, 0, 0, 1)';
        if('text_font' in this.default_option === true) this.elements.ctx.font = (Number(this.default_option.text_font.slice(0, 2)) * this.ratio).toString() + this.default_option.text_font.slice(2);
        if('text_line' in this.default_option === true) this.elements.ctx.textBaseline = this.default_option.text_line;
        if('text_align' in this.default_option === true) this.elements.ctx.textAlign = this.default_option.text_align;
        this.elements.ctx.fillText(this.default_option.text_content, this.adjustX, this.adjustY, this.adjustWidth);
      }
      if('border' in this.default_option === true){
        this.elements.ctx.lineWidth = this.default_option.border * this.ratio;
        if('border_color' in this.default_option === true) this.elements.ctx.strokeStyle = this.default_option.border_color;
        this.elements.ctx.strokeRect(this.adjustX, this.adjustY, this.adjustWidth, this.adjustHeight);
      }
    }
...
class Elements {
  ...
  createSprite(x, y, width, height){
    ...
    sprite.elements = this;
    return sprite;
  }

  set(){
    this.canvas.id = 'canvas';
    this.getWindowSize(window.innerWidth, window.innerHeight - 10);
  ...
  start(){
    this.setFps(50);
    if(this.animationMode === 'request') this.loop = window.requestAnimationFrame(this.update.bind(this));
    else this.loop = setTimeout(this.update.bind(this), this.mspf);
    console.log('start');
  }
  ...
  update(){
    ...
    if(this.animationMode === 'request'){
      ...
      if(this.end === false) this.loop = window.requestAnimationFrame(this.update.bind(this));
    } else {
      for(var element of this.childs){
        if(element.updateEvent) element.update();
      }
      ...
      if(this.end === false) this.loop = setTimeout(this.update.bind(this), this.mspf);
    }
  }
